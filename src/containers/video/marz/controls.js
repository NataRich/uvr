 ...
 */
 ...
 const bindControls = (videoInstance) => {
     var video = videoInstance.element;
     ...
     // Handle events on the video element. videoInstance automatically reattaches
     // handlers to new video when the resolution changes
     videoInstance.addEventListenerVideo('timeupdate', updateProgressBar);
     videoInstance.addEventListenerVideo('pause', updatePlayPause);
     videoInstance.addEventListenerVideo('play', updatePlayPause);
     videoInstance.addEventListenerVideo('playing', updatePlayPause);
     videoInstance.addEventListenerVideo('timeupdate', updateCurrentTimeIndicator);
     videoInstance.addEventListenerVideo('volumechange', updateMute);

     // Handle resolution change
     videoInstance.addEventListener('change', updateCurrentTimeIndicator);
     videoInstance.addEventListener('change', updateDurationIndicator);
     videoInstance.addEventListener('change', updateProgressBar);
     videoInstance.addEventListener('change', updatePlayPause);
     videoInstance.addEventListener('change', updateMute);
     ...
     function setResolutions() {
         videoInstance.resolutions().forEach(function (level, i) {
             ...
             levelElement.addEventListener('click', function () {
                 videoInstance.setResolutionIndex(i);
             });
     ...
     function updateResolutionIndicator() {
         var level = videoInstance.resolution();
     ...
     function updateSelectedResolution() {
         for (var i = 0; i < resolutionSelectElement.children.length; i++) {
             ...
             if (i === videoInstance.resolutionIndex()) {
     ...
     function updateResolutionChanging() {
         if (videoInstance.resolutionChanging()) {
     ...
     videoInstance.addEventListener('change', updateResolutionIndicator);
     videoInstance.addEventListener('change', updateSelectedResolution);
     videoInstance.addEventListener('change', updateResolutionChanging);
